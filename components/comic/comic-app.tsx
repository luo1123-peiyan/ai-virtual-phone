...
// 漫画源列表：对齐 Venera 首页。当前仅「拷贝漫画」已接入后端，其余为占位。
...
function NavIcon({ path, active, label, onClick }: { path: string; active: boolean; label: string; onClick: () => void }) {
  return (
    <button
      ...
      aria-label={label}
      className={
        "flex flex-1 items-center justify-center py-2 " + (active ? "text-blue-600" : "text-gray-400")
      }
    >
...
export default function ComicApp({ onClose }: Props) {
            ...
            {SOURCES.map((source) => (
              <button
                ...
                className={
                  ...
                  (source.active ? "bg-blue-600 text-white" : "bg-blue-50 text-blue-700")
                }
              >
                {source.name}
              </button>
  ...
  function renderExplore() {
    return (
      <div className="flex flex-1 flex-col overflow-hidden">
        <div className="flex flex-none items-center gap-2 border-b px-3 pb-2 pt-3">
          <button type="button" onClick={() => setView("home")} className="flex h-8 w-8 items-center justify-center text-gray-500" aria-label="返回">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
          </button>
